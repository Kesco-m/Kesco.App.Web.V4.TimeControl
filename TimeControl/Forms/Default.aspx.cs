...
namespace Kesco.App.Web.TimeControl.Forms
{
    ...
    public partial class Default : BasePage
    {
        ...
        private bool DetailsByDay
        {
            get
            {
                return (Period.ValuePeriod == ((int)PeriodsEnum.Day).ToString(CultureInfo.InvariantCulture)) ||
                    (Period.ValuePeriod == ((int)PeriodsEnum.Custom).ToString(CultureInfo.InvariantCulture) &&
                    Period.ValueDateFrom == Period.ValueDateTo);
            }
        }

        private EmplTableCash SourceCash
        {
            get { return PrimaryEmployeeCalc ? EmplListCashEmpl : EmplListCashComp; }
        }
        ...
        private EmplTableCash EmplListCashEmpl
        {
            get
            {
                if (Etc == null || !Etc.PrimaryEmployeeCalc)
                {
                    Etc = new EmplTableCash(GetDataTable(), true, this);
                }
                return Etc;
            }
        }
        ...
        private EmplTableCash EmplListCashComp
        {
            get
            {
                if (Etc == null || Etc.PrimaryEmployeeCalc)
                {
                    Etc = new EmplTableCash(GetDataTable(), false, this);
                }
                return Etc;
            }
        }
        ...
        private SortInfoSetting SortParams
        {
            get
            {
                if (Info == null)
                {
                    Info = new SortInfoSetting();
                }

                if (Info.Count == 0)
                {
                    Info.Insert(0, new SortInfoItem(SortType.Asc, "EmplNameSort"));
                }
                return Info;
            }
        }
        ...
        private string SortString
        {
            get
            {
                ...
                foreach (SortInfoItem itemp in SortParams)
                {
                    ...
                    if (itemp.ItemSortType == SortType.Desc)
                    {
                        result = result + " DESC";
                    }
                }
        ...
        private bool NeedToDisplayAdditionCols
        {
            get { return DetailsByDay; }
        }
        ...
        protected override void ProcessCommand(string cmd, NameValueCollection param)
        {
            switch (cmd)
            {
                ...
                case "ClearFilter":
                    ClearFilter();
                    break;
                case "openDetails": RedirectToDetails(param["id"], param["from"]); break;
                case "openDaysDetails": RedirectToDaysDetails(param["id"], param["from"], param["to"], param["period"]); break;
            }
        ...
        private void RedirectToDetails(string empl, string startDate)
        {
            ...
            JS.Write("v4_windowOpen('TimeDetailsForm.aspx?id={0}&date={1}&prim={2}&personid={3}&tz={4}', '_blank', 'left=340,top=240,width=750,height=700,resizable=yes,scrollbars=yes');",
        ...
        {
            JS.Write("window.location.assign('EmplDaysDetailsForm.aspx?id={0}&from={1}&to={2}&period={3}&prim={4}&tz={5}');",
                empl, HttpUtility.UrlEncode(startDate), HttpUtility.UrlEncode(endDate), periodType, PrimaryEmployeeCalc, Tz);
        }

        private void ClearFilter()
        {
            Company.ValueSelectEnum = Subdivision.ValueSelectEnum = Position.ValueSelectEnum = EmplName.ValueSelectEnum =
                Subdivision.Filter.PcId.CompanyHowSearch = EmplName.Filter.IdsCompany.CompanyHowSearch = EmplName.Filter.SubdivisionIDs.SubdivisionHowSearch =
        ...
        {
            return String.Format("<A href=\"#\" onclick=\"cmd('cmd', 'setSortOrder', 'col', 'StartTime');\">{0}</A>&nbsp&nbsp&nbsp<A href=\"#\" onclick=\"cmd('cmd', 'setSortOrder', 'col', 'StartTime');\"><FONT style='font-size:7pt; color:#C00000;'>{1}</FONT></A>", Resx.GetString("cStartTime"), SortTagStartTime);
        }

        protected string GetEndTimeHeaderText()
        {
            return String.Format("<A href=\"#\" onclick=\"cmd('cmd', 'setSortOrder', 'col', 'EndTime');\">{0}</A>&nbsp&nbsp&nbsp<A href=\"#\" onclick=\"cmd('cmd', 'setSortOrder', 'col', 'EndTime');\"><FONT style='font-size:7pt; color:#C00000;'>{1}</FONT></A>", Resx.GetString("cEndTime"), SortTagEndTime);
        }
        ...
        {
            int emplId = (int)row["EmplID"];
            int personId = (int)row["PersonId"];
            object linkCell;
            string from = Period.ValueDateFrom.HasValue ? Period.ValueDateFrom.Value.ToString("dd.MM.yyyy") : "";
            string from2 = Period.ValueDateFrom.HasValue ? Period.ValueDateFrom.Value.AddDays(1).ToString("dd.MM.yyyy") : "";
            string to = Period.ValueDateTo.HasValue ? Period.ValueDateTo.Value.ToString("dd.MM.yyyy") : "";
            if (DetailsByDay)
            {
                linkCell = String.Format("<A href=\"#\" onclick=\"cmd('cmd', 'openDetails', 'id', '{0}', 'from', '{1}', 'to', '{2}');return false;\"><IMG src=\"{3}detail.GIF\" border=\"0\" title=\"{4}\"></A>",
                    emplId, from, from2, Global.Styles, Resx.GetString("hPageSubTitleDetails2"));
            }
            else
            {
                linkCell = String.Format("<A href=\"#\" onclick=\"cmd('cmd', 'openDaysDetails', 'id', '{0}', 'from', '{1}', 'to', '{2}', 'period', '{3}');return false;\"><IMG src=\"{4}detail.GIF\" border=\"0\" title=\"{5}\"></A>",
                        emplId, from, to, Period.ValuePeriod, Global.Styles, Resx.GetString("hPageDetailsTitle"));
            }
            object employeeCell = String.Format("<A href=\"#\" onclick=\"ItemClick('{0}', '{3}');\" class=\"v4_callerControl\" caller-type=\"2\" data-id=\"{0}\"  onmouseout=\"mouseOut();\">{1}</A>", emplId, row["EmplNameSort"], personId, Lib.Web.Settings.Config.user_form);

            bool isShowLink = !((int)((TimeSpan)row[8]).TotalSeconds == 0 && (int)((TimeSpan)row[9]).TotalSeconds == 0);
        ...
        {
            string stm = TimeExit.Value;
            if (stm.Length == 0) stm = "00:00:00";
            stm = stm.Replace(":", "");
            DateTime temp = Period.ValueDateFrom ?? new DateTime(1753, 1, 2);
            DateTime tm = Str2DateTime(temp.ToString("yyyyMMdd") + stm);
            
            if (DetailsByDay)
            {
                SourceCash.CompleteEmplTable(Period.ValueDateFrom, Period.ValueDateFrom, EmplName.SelectedItemsString, Company.SelectedItemsString, Position.SelectedItemsString, Subdivision.SelectedItemsString,
                                             Filter.TimeExitAvaible, tm, Filter.EmplAvaible, Tz, Company.ValueSelectEnum, Subdivision.ValueSelectEnum,
                                             Position.ValueSelectEnum, EmplName.ValueSelectEnum, CurrentPageSetting, RowsPerPageSetting, FilterSubEmpl.Checked);
            }
            else
            {
                SourceCash.CompleteEmplTable(Period.ValueDateFrom, Period.ValueDateTo, EmplName.SelectedItemsString, Company.SelectedItemsString, Position.SelectedItemsString, Subdivision.SelectedItemsString,
                                             Filter.TimeExitAvaible, tm, Filter.EmplAvaible, Tz, Company.ValueSelectEnum, Subdivision.ValueSelectEnum,
                                             Position.ValueSelectEnum, EmplName.ValueSelectEnum, CurrentPageSetting, RowsPerPageSetting, FilterSubEmpl.Checked);
            }

            DataTable tAllRows = SourceCash.GetSourceTable();
            DataTable tPeriodsList = GetDataTable();

            //_totalRecord = tAllRows.Rows.Count;
            _totalRecord = SourceCash.CountEmployee;

            string sortTagEmplName = String.Empty;
            string sortTagAbsentTime = String.Empty;
            string sortTagInterval = String.Empty;
            string sortTagInternetAccessCount = String.Empty;
            string sortTagInternetAccessTotal = String.Empty;

            foreach (SortInfoItem item in SortParams)
            {
                if (SortParams.IndexOf(item) == 0)
                {
                    string strUpEnabledGif = String.Format("<IMG SRC=\"{0}ScrollUpEnabled.GIF\" border=0>", Global.Styles);
                    string strDownEnabledGif = String.Format("<IMG SRC=\"{0}ScrollDownEnabled.gif\" border=0>", Global.Styles);
            ...
            BcEmplName.HeaderText = String.Format("<a href=\"#\" onclick=\"cmd('cmd', 'setSortOrder', 'col', 'EmplNameSort');\">{0}</a>&nbsp&nbsp&nbsp<a href=\"#\" onclick=\"cmd('cmd', 'setSortOrder', 'col', 'EmplNameSort');\"><font style='font-size:7pt; color:#C00000;'>{1}</font></a>", Resx.GetString("cEmplName"), sortTagEmplName);
            BcAbsentTime.HeaderText = String.Format("<a href=\"#\" onclick=\"cmd('cmd', 'setSortOrder', 'col', 'AbsentTimeSort');\">{0}</a>&nbsp&nbsp&nbsp<a href=\"#\" onclick=\"cmd('cmd', 'setSortOrder', 'col', 'AbsentTimeSort');\"><font style='font-size:7pt; color:#C00000;'>{1}</font></a>", Resx.GetString("сAbsent"), sortTagAbsentTime);
            BcInterval.HeaderText = String.Format("<a href=\"#\" onclick=\"cmd('cmd', 'setSortOrder', 'col', 'IntervalSort');\">{0}</a>&nbsp&nbsp&nbsp<a href=\"#\" onclick=\"cmd('cmd', 'setSortOrder', 'col', 'IntervalSort');\"><font style='font-size:7pt; color:#C00000;'>{1}</font></a>", Resx.GetString("cInterval"), sortTagInterval);
            BcInternetAccessCount.HeaderText = String.Format("<a href=\"#\" onclick=\"cmd('cmd', 'setSortOrder', 'col', 'InternetAccessCount');\">{0}</a>&nbsp&nbsp&nbsp<a href=\"#\" onclick=\"cmd('cmd', 'setSortOrder', 'col', 'InternetAccessCount');\"><font style='font-size:7pt; color:#C00000;'>{1}</font></a>", Resx.GetString("cInternetAccessCount"), sortTagInternetAccessCount);
            BcInternetAccessTotalTime.HeaderText = String.Format("<a href=\"#\" onclick=\"cmd('cmd', 'setSortOrder', 'col', 'InternetAccessTotaltime');\">{0}</a>&nbsp&nbsp&nbsp<a href=\"#\" onclick=\"cmd('cmd', 'setSortOrder', 'col', 'InternetAccessTotaltime');\"><font style='font-size:7pt; color:#C00000;'>{1}</font></a>", Resx.GetString("cInternetAccessTotalTime"), sortTagInternetAccessTotal);
            

            int pageNo = pagerBar.CurrentPageNumber - 1;
            int pageRowCount = pagerBar.RowsPerPage;

            DataRow[] rows = tAllRows.Select("", SortString);
            //int lastPageNo = (rows.Length - 1) / pageRowCount;
            int lastPageNo = (_totalRecord - 1) / pageRowCount;
        ...
        private void InitBindColumns()
        {
            foreach (DataGridColumn bcol in intervalList.Columns)
            {
                if (bcol is BoundColumn)
                {
                    ...
                    {
                        case "EmplNameCol": BcEmplName = col; break;
                        case "StartTime": BcStartTime = col; break;
                        case "EndTime": BcEndTime = col; break;
                        case "AbsentTime": BcAbsentTime = col; break;
                        case "Interval": BcInterval = col; break;
                        case "LinkCell":
                            //col.HeaderImageUrl = Global.Styles + "Print.gif";
                            BcLinkCell = col; break;
                        case "InternetAccessCount": BcInternetAccessCount = col; break;
                        case "InternetAccessTotaltime": BcInternetAccessTotalTime = col; break;
                        case "isEnterAfterExit": BcIsEnterAfterExit = col; break;
                        case "isEnterAfterExit2": BcIsEnterAfterExit2 = col; break;
                    }
                }
            }
        }
        ...
        private void SetProperty()
        {
            ...
            if (CurrentUser.Language.ToLower() == "ru")
            {
                fio = "FullName";
            }
            ...
            HelpUrl = Request.Url.Scheme + "://" + Request.Url.Host + Request.ApplicationPath + "/Forms/hlp/help.htm?id=0";
        ...
        {
            SetGrayStyle();
            JS.Write("displayTimeExit({0}, {1});", Period.ValuePeriod == ((int)PeriodsEnum.Day).ToString(CultureInfo.InvariantCulture) ||
                (Period.ValueFrom.Length > 0 && Period.ValueDateFrom == Period.ValueDateTo) ? 1 : 0, inx.Length == 0 ? "0" : inx);
        }
        ...
        private void RefreshListInternal()
        {
            ...
            else
            {
                ...
                if (SourceCash.GetSourceTable().Rows.Count > 0 &&
                    Period.ValuePeriod != ((int) PeriodsEnum.Undefined).ToString(CultureInfo.InvariantCulture) && Period.ValueDateTo != null && !Period.ValueDateTo.Value.Equals(String.Empty) &&
                    ((Period.ValueDateFrom != null && !Period.ValueDateFrom.Value.Equals(String.Empty)) || Period.ValuePeriod == ((int) PeriodsEnum.Day).ToString(CultureInfo.InvariantCulture)))
                {
                    intervalList.RenderControl(writer);
                    String tableContent = stringWriter.ToString().Replace("\n", "").Replace("\r", "").Replace("\t", "").Replace("'", "\\'");
                    JS.Write(String.Format("document.all('listTable').innerHTML = '{0}';", tableContent));
                    pagerBar.SetDisabled(false);
                }
                else
                {
                    JS.Write(String.Format("document.all('listTable').innerHTML = '<P align=\\'center\\'><BR>{0}</P>';", Resx.GetString("lNoData")));
        ...
        protected void Print()
        {
            ...
            if (!String.IsNullOrEmpty(idpage))
            {
                ...
                if (p != null)
                {
                    EmployeePeriodsInfoDs ds = ((Default)p).SourceCash.GetDS();
                    if (ds.Сотрудники.Rows.Count > ((Default)p).RowsPerPageSetting)
                    {
                        DateTime startPeriod = ((Default)p).Period.ValueDateFrom ?? new DateTime(1753, 1, 2);
                        DateTime endPeriod = ((Default)p).Period.ValueDateTo ?? new DateTime(1753, 1, 2);
                        ((Default)p).SourceCash.FillPage(ds, startPeriod, endPeriod, 1, ds.Сотрудники.Rows.Count);
                    }
                    DataTable dt = ((Default)p).SourceCash.GetSourceTable();
                    if (dt.Rows.Count > 0)
                    {
                        if (((Default)p).Period.ValuePeriod == ((int)PeriodsEnum.Day).ToString(CultureInfo.InvariantCulture) ||
                            (((Default)p).Period.ValuePeriod == ((int)PeriodsEnum.Custom).ToString(CultureInfo.InvariantCulture) &&
        ...
        {
            WrResponse = String.Format("<html><head><title>{0}</title></head><body onload=\"window.print()\">", Resx.GetString("lPrint"));
            WrResponse += "<b>" + Resx.GetString("hPageTitle") + "</b> (" + DateTime.Now.ToString("yyyy.MM.dd HH:mm:ss") + ")";
            WrResponse += "<table style=\"border-collapse:collapse;\">";
            WrResponse += String.Format("<tr style=\"height: 25px; font-weight: bold; text-align: center;\"><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td></tr>",
                Resx.GetString("cEmplName"), Resx.GetString("cStartTime"), Resx.GetString("cEndTime"), Resx.GetString("cInterval"), Resx.GetString("сAbsent"));
            foreach (DataRow row in dt.Rows)
            {
                ...
                WrResponse += "<tr>";
                WrResponse += "<td style=\"border-bottom: darkgray 1px solid;border-right: darkgray 1px solid;\">" + row[10] + "</td>";
                WrResponse += "<td style=\"text-align: center;border-bottom: darkgray 1px solid;border-right: darkgray 1px solid;\">" + start + "</td>";
                WrResponse += "<td style=\"text-align: center;border-bottom: darkgray 1px solid;border-right: darkgray 1px solid;\">" + end + "</td>";
                WrResponse += "<td style=\"text-align: center;border-bottom: darkgray 1px solid;border-right: darkgray 1px solid;\">" + work + "</td>";
                WrResponse += "<td style=\"text-align: center;border-bottom: darkgray 1px solid;\">" + notwork + "</td>";
                WrResponse += "</tr>";
            }
            WrResponse += "</table>";
            if (((Default)p).SourceCash.IsError)
            {
        ...
        private void FillPrintData(DataTable dt, Page p)
        {
            WrResponse = String.Format("<html><head><title>{0}</title></head><body onload=\"window.print()\">", Resx.GetString("lPrint"));
            WrResponse += "<b>" + Resx.GetString("hPageTitle") + "</b> (" + DateTime.Now.ToString("yyyy.MM.dd HH:mm:ss") + ")";
            WrResponse += "<br />За период: " + (((Default)p).Period.ValueDateFrom.HasValue ? ((Default)p).Period.ValueDateFrom.Value.ToString("yyyy.MM.dd") : "") + " - " +
                (((Default)p).Period.ValueDateTo.HasValue ? ((Default)p).Period.ValueDateTo.Value.ToString("yyyy.MM.dd") : "");
            WrResponse += "<table style=\"border-collapse:collapse;\">";
            WrResponse += String.Format("<tr style=\"height: 25px; font-weight: bold; text-align: center;\"><td>{0}</td><td>{1}</td></tr>",
        ...
        private DataTable GetDataTable()
        {
            ...
            EmplID = new DataColumn("EmplID", typeof(Int32));
            PersonID = new DataColumn("PersonID", typeof(Int32));
        ...
        private void GetParams()
        {
            ...
            string[] arr = Filter.CompanyItems.Split(new[]{","}, StringSplitOptions.RemoveEmptyEntries);
            foreach (var id in arr)
            {
                Company.SelectedItems.Add(new Lib.Entities.Item { Id = id, Value = new Person(id) });
            }
            arr = Filter.SubdivisionItems.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);
            foreach (var id in arr)
            {
                Subdivision.SelectedItems.Add(new Lib.Entities.Item { Id = id, Value = new Subdivision{Id = id, Name = id} });
            }
            arr = Filter.PositionItems.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);
            foreach (var id in arr)
            {
                Position.SelectedItems.Add(new Lib.Entities.Item { Id = id, Value = new Position { Id = id, Name = id } });
            }
            arr = Filter.PersonItems.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);
            foreach (var id in arr)
            {
                EmplName.SelectedItems.Add(new Lib.Entities.Item { Id = id, Value = new Employee(id) });
            }

            Company.ValueSelectEnum = Subdivision.Filter.PcId.CompanyHowSearch = Position.Filter.PcId.CompanyHowSearch = EmplName.Filter.IdsCompany.CompanyHowSearch = Filter.PeriodInfo.CompanyHowSearch;
            Subdivision.ValueSelectEnum = Position.Filter.SubdivisionIDs.SubdivisionHowSearch = EmplName.Filter.SubdivisionIDs.SubdivisionHowSearch = Filter.PeriodInfo.SubdivisionHowSearch;
            Position.ValueSelectEnum = EmplName.Filter.PositionIDs.PositionHowSearch = Filter.PeriodInfo.PositionHowSearch;
            EmplName.ValueSelectEnum = Filter.PeriodInfo.PersonHowSearch;

            Subdivision.Filter.PcId.Value = Position.Filter.PcId.Value = EmplName.Filter.IdsCompany.Value = Company.SelectedItemsString;
            Position.Filter.SubdivisionIDs.Value = EmplName.Filter.SubdivisionIDs.Value = Subdivision.SelectedItemsString;
        ...
        private void SetGrayStyle()
        {
            ...
            if (JS.ToString().Contains("if (document.all('intervalList')) document.all('intervalList').className = 'Grid8Grayed';"))
                return;
        ...
        private void CheckSubdivision()
        {
            ...
            if (String.IsNullOrEmpty(Subdivision.Filter.PcId.Value) && Subdivision.Filter.PcId.CompanyHowSearch != "3") return;
            for (int i = 0; i < Subdivision.SelectedItems.Count; i++)
            {
                if (!new ExecQuery().CheckSubdivisionByCompanyId(Subdivision.Filter.PcId.Value, Subdivision.SelectedItems[i].Id, Subdivision.Filter.PcId.CompanyHowSearch == "1",
                    Subdivision.Filter.PcId.CompanyHowSearch == "3"))
                {
                    Subdivision.SelectedItems.Remove(Subdivision.SelectedItems.Find(x => x.Id == Subdivision.SelectedItems[i].Id));
                    i--;
                }
            }
            EmplName.Filter.SubdivisionIDs.Value = Position.Filter.SubdivisionIDs.Value = Subdivision.SelectedItemsString;
        ...
        private void CheckPosition()
        {
            ...
            if (String.IsNullOrEmpty(Position.Filter.PcId.Value) && Position.Filter.PcId.CompanyHowSearch != "3") return;
            for (int i = 0; i < Position.SelectedItems.Count; i++)
            {
                if (!new ExecQuery().CheckPositionByCompanyId(Position.Filter.PcId.Value, Position.SelectedItems[i].Id, Position.Filter.PcId.CompanyHowSearch == "1",
                    Position.Filter.PcId.CompanyHowSearch == "3"))
                {
                    Position.SelectedItems.Remove(Position.SelectedItems.Find(x => x.Id == Position.SelectedItems[i].Id));
                    i--;
                }
            }
            EmplName.Filter.PositionIDs.Value = Position.SelectedItemsString;
        ...
        private void CheckPerson()
        {
            ...
            if (String.IsNullOrEmpty(EmplName.Filter.IdsCompany.Value) && EmplName.Filter.IdsCompany.CompanyHowSearch != "3") return;
            for (int i = 0; i < EmplName.SelectedItems.Count; i++)
            {
                if (!new ExecQuery().CheckPersonByCompanyId(EmplName.Filter.IdsCompany.Value, EmplName.SelectedItems[i].Id, EmplName.Filter.IdsCompany.CompanyHowSearch == "1",
                    EmplName.Filter.IdsCompany.CompanyHowSearch == "3"))
                {
                    EmplName.SelectedItems.Remove(EmplName.SelectedItems.Find(x => x.Id == EmplName.SelectedItems[i].Id));
                    i--;
                }
            }
        ...
        private void CheckPositionBySubdivision()
        {
            ...
            if (String.IsNullOrEmpty(Position.Filter.SubdivisionIDs.Value) && Position.Filter.SubdivisionIDs.SubdivisionHowSearch != "3") return;
            for (int i = 0; i < Position.SelectedItems.Count; i++)
            {
                if (!new ExecQuery().CheckPositionBySubdivisionId(Position.Filter.SubdivisionIDs.Value, Position.SelectedItems[i].Id, Position.Filter.SubdivisionIDs.SubdivisionHowSearch == "1",
        ...
        private void CheckPersonBySubdivision()
        {
            ...
            if (String.IsNullOrEmpty(EmplName.Filter.SubdivisionIDs.Value) && EmplName.Filter.SubdivisionIDs.SubdivisionHowSearch != "3") return;
            for (int i = 0; i < EmplName.SelectedItems.Count; i++)
            {
                if (!new ExecQuery().CheckPersonBySubdivisionId(EmplName.Filter.SubdivisionIDs.Value, EmplName.SelectedItems[i].Id, EmplName.Filter.SubdivisionIDs.SubdivisionHowSearch == "1",
        ...
        private void CheckPersonByPosition()
        {
            ...
            if (String.IsNullOrEmpty(EmplName.Filter.PositionIDs.Value) && EmplName.Filter.PositionIDs.PositionHowSearch != "3") return;
            for (int i = 0; i < EmplName.SelectedItems.Count; i++)
            {
                if (!new ExecQuery().CheckPersonByPositionId(EmplName.Filter.PositionIDs.Value, EmplName.SelectedItems[i].Id, EmplName.Filter.PositionIDs.PositionHowSearch == "1",
